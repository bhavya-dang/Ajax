...
module.exports.run = async (bot, message, args) => {
  ...
  .setAuthor("41 Total Commands", bot.user.displayAvatarURL)
  .setColor("#c5f4a6")
  ...
  .addField("**Utility Commands**", "`serverstats`|`stats`|`userstats`|`tempmute`|`weather`|`ping`|`say`|`docs`\n`clear`|`reload`|`add`|`subtract`|`setgame`|`define`|`icon`|`shorten`|`find``auto`")
  .addField("**User Commands**", "`coins`|`level`|`warnlvl`|`pay`|`avatar`")
  .addField("**Fun Commands**","`8ball`|`poll`|`roll`|`test`|`doggo`|`kitten`|`flip`|`slots`|`joke`|`ascii`|`poke`", true)
  .addField("**Server:** https://discord.gg/YnBCs4D","Website: https://tritaxgamer.github.io/tritax/", true)
  .setThumbnail(icon);
